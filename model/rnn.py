...
class SubjectObjectRelationModel(object):
...
class PositionAwareRNN(nn.Module):
    ...
    def __init__(self, opt, emb_matrix=None):
        ...
        if opt['dep_dim'] > 0:
            self.dep_emb = nn.Embedding(len(constant.DEP_TO_ID), opt['dep_dim'],
                                        padding_idx=constant.PAD_ID)

        input_size = opt['emb_dim'] + opt['pos_dim'] + opt['dep_dim']
    ...
    def init_weights(self):
        ...
        if self.opt['dep_dim'] > 0:
            self.dep_emb.weight.data[1:, :].uniform_(-1.0, 1.0)
    ...
    def forward(self, inputs):
        words, masks, pos, dep, subj_pos, obj_pos = inputs  # unpack
